...
class ModelArguments:
    ...
    mm_min_image_token:Optional[int] = field(default=4)
    mm_max_image_token:Optional[int] = field(default=2048)
    resize_image_size:Optional[int] = field(default=None)
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
        return image, image_size, "image"#image_size是原分辨率
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        processor = self.data_args.image_processor
        if hasattr(processor, 'tokenizer_class') and 'Qwen2Tokenizer' in str(processor.tokenizer_class):
            packing=True
        else:
            packing=False
        ...
        elif "video" in sources[0]:#!video先不管
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image)#pt:组织成<image>T T T\n的格式
...
class DataCollatorForSupervisedDataset(object):
    ...
    tokenizer: transformers.PreTrainedTokenizer
    processor: Optional[Any] = None  # 泛化类型
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        if any(name in self.processor.__class__.__name__ for name in ["CLIPImageProcessor", "SigLipImageProcessor"]):#!这段代码需要优化来支持更多的ViT
            ...
            if "image" in instances[0]:
                images = [instance['image'] for instance in instances if instance['image'] is not None]#这是路径
                batch["modalities"] = [im[1] for im_list in images for im in im_list]
                images = [im[0] for im_list in images for im in im_list]
                ...
                batch['packing'] = True
                batch["image_sizes"] = None#qwen-vit用不到
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        cfg_pretrained = AutoConfig.from_pretrained(model_args.model_name_or_path)#Load LLM

    if model_args.use_pos_skipping is not None and model_args.pos_skipping_range is not None:
        overwrite_config["use_pos_skipping"] = model_args.use_pos_skipping #pt:False
        overwrite_config["pos_skipping_range"] = model_args.pos_skipping_range#pt:4096
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if "moe" in model_args.model_name_or_path.lower() or "A14B" in model_args.model_name_or_path:
                ...
                model = LlavaQwenForCausalLM.from_pretrained(#仅加载llm
                    model_args.model_name_or_path,
                    cache_dir=training_args.cache_dir,
                    attn_implementation=training_args.attn_implementation,
                    torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
...
def train(attn_implementation=None):
    ...
    model = get_model(model_args, training_args, bnb_model_from_pretrained_args)#初始化LLM
    ...
    if model_args.vision_tower is not None:
        model.get_model().initialize_vision_modules(model_args=model_args, fsdp=training_args.fsdp)#not delayed
        ...
        model.config.image_aspect_ratio = data_args.image_aspect_ratio#pt:square
        if data_args.image_grid_pinpoints is not None:#pt的时候用不到
        ...
        model.config.image_grid_pinpoints = data_args.image_grid_pinpoints#pt:None
        model.config.image_crop_resolution = data_args.image_crop_resolution#pt:None
        model.config.image_split_resolution = data_args.image_split_resolution#pt:None
        model.config.tokenizer_padding_side = tokenizer.padding_side
        model.config.tokenizer_model_max_length = tokenizer.model_max_length#pt:8192#!我看blog中的设置貌似是4096?
        model.config.mm_newline_position = model_args.mm_newline_position#pt:grid
        model.config.add_faster_video = model_args.add_faster_video#pt:false
        model.config.faster_token_stride = model_args.faster_token_stride#pt:10
        model.config.add_time_instruction = data_args.add_time_instruction#pt:false
        model.config.force_sample = data_args.force_sample#pt:false
        model.config.mm_spatial_pool_stride = model_args.mm_spatial_pool_stride #pt:None
